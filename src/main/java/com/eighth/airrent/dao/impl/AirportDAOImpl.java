...
public class AirportDAOImpl extends BaseDAO implements AirportDAO {
	...
	@Override
	public OpenPage<Airport> findAirportList(OpenPage<Airport> openPage,String airportName) {
		StringBuffer sql=new StringBuffer();
		sql.append("select count(*) from t_airrent_airport where airport_name like '%"+airportName+"%'");
		int count = getJdbcTemplate().queryForInt(sql.toString());
		if(count>0){
			openPage.setTotal(count);
			sql=new StringBuffer();
			sql.append("select * from t_airrent_airport where airport_name like '%"+airportName+"%' limit "+openPage.getPageSize()+" OFFSET "+(openPage.getFirst() - 1)+"");
	...
	public OpenPage<Plane> findPlaneByAirportId(OpenPage openPage,
			String airportId) {
		StringBuffer sql=new StringBuffer();
		sql.append("select count(*) from t_airrent_plane where airport_id='"+airportId+"'");
		int count = getJdbcTemplate().queryForInt(sql.toString());
		if(count>0){
			openPage.setTotal(count);
			sql=new StringBuffer();
			sql.append("select * from t_airrent_plane where airport_id='"+airportId+"' limit "+openPage.getPageSize()+" OFFSET "+(openPage.getFirst() - 1)+"");
